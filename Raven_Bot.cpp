...
{
    // 1. 뇌(Brain) 업데이트 - 여기서 도망갈지, 약 먹을지 결정함
    m_pBrain->Process();

    // 2. 이동(Movement) 업데이트
    UpdateMovement();

    // 3. 봇이 사람 손(Possessed)에 있지 않을 때만 AI 작동
    if (!isPossessed())
    {
        // 적 탐색
        ...
        // 조건 1: !m_bHoldFire (Goal에서 강제로 끄지 않았을 때)
        // 조건 2: Health() > 30 (체력이 30보다 많을 때만 쏨)
        // ------------------------------------------------------------------
        if (!m_bHoldFire && Health() > 30)
        {
...
{
    // m_iHealth -= val; // 이거 두 번 들어가 있어서 하나 뺐습니다.
